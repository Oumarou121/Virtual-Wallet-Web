...
class retraitConfirm extends StatefulWidget {
  retraitConfirm(
      {super.key,
 ...
 final int Montant;
  @override
  State<retraitConfirm> createState() => _retraitConfirmState();
}

class _retraitConfirmState extends State<retraitConfirm> {
  String error = '';
      ...
      child: ElevatedButton(
        onPressed: () async {
          if (onClick = true) {
            ...
            var collectionS =
            FirebaseFirestore.instance.collection('Retrait $MyUid');
            var collectionR = FirebaseFirestore.instance.collection('Retrait $YourUid');
            if (widget.Montant <= userDataR.waterCounter) {
  ...
  Widget build(BuildContext context) {
    var collectionM =
  FirebaseFirestore.instance.collection('Confirm ${widget.IdR}');
    final user = Provider.of<AppUser?>(context);
    if (user == null) throw Exception("user not found");
    final databaseS = DatabaseService(user.uid);
    final databaseR = DatabaseService(widget.IdR);
    return FutureBuilder(
      future: MessageLenght(collection: collectionM),builder: (context, snapshot) => StreamProvider<AppUserData?>.value(
                      ...
                      child: StreamBuilder<AppUserData>(
                          ...
                          builder: (context, snapshot) {
                            if (snapshot.hasData) {
                              ...
                              return  items.length == 2 ? Scaffold(
                                body: Column(
                                  children: [        SizedBox(
                                    height: MediaQuery.of(context).size.height / 3,
                                  ),
                                    Center(child: Text('Chargement',style: TextStyle(fontSize: 32),)), const SizedBox(height: 20),
                                    AvatarGlow(
                                              ...
                                              child: Column(
                                                children: [
                                                  Text(
                                                      ...
                                                      SwicthLangues(
                                                          userData: userDataS,
                                                          fr: "Voulez-vous vraiment ce retrait",
                                                          en: "Do you really want to retrait")),
                                                  SizedBox(height: 15),
                                                  _SendConfirm(
